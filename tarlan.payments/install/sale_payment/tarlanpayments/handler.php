    ...
    extends PaySystem\ServiceHandler
{
    ...
    private function createInvoice(Payment $payment): array
    {
        ...
        $projectReferenceID = $payment->getField("ID") . ":" . $this->guidv4();

        $hash = $this->prepareHash($payment, $projectReferenceID, $amount);
    ...
    private function prepareHash(Payment $payment, string $projectReferenceID, string $amount): string
    {
        $projectHash = $projectReferenceID . $amount;
